...
def VAD_chunk(aggressiveness, path):
    audio, byte_audio = read_wave(path, param.data.sr)
    vad = webrtcvad.Vad(int(aggressiveness))
    frames = frame_generator(20, byte_audio, param.data.sr)
    frames = list(frames)
    times = vad_collector(param.data.sr, 20, 200, vad, frames)
    speech_times = []
    speech_segs = []
    for i, time in enumerate(times):
        ...
        while j + .4 < end:
            ...
            speech_segs.append(audio[int(j * param.data.sr):int(end_j * param.data.sr)])
            j = end_j
        else:
            speech_times.append((j, end))
            speech_segs.append(audio[int(j * param.data.sr):int(end * param.data.sr)])
