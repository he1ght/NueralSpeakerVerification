...
def get_STFTs(segs):
    ...
    STFT_frames = []
    for seg in segs:
        S = librosa.core.stft(y=seg, n_fft=param.data.nfft,
                              win_length=int(param.data.window * sr), hop_length=int(param.data.hop * sr))
        S = np.abs(S) ** 2
        mel_basis = librosa.filters.mel(sr, n_fft=param.data.nfft, n_mels=param.data.nmels)
        S = np.log10(np.dot(mel_basis, S) + 1e-6)  # log mel spectrogram of utterances
        for j in range(0, S.shape[1], int(.12 / param.data.hop)):
            if j + 24 < S.shape[1]:
