...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train_dataset, batch_size=param.train.batch, shuffle=True,
    ...
    for e in range(param.train.epochs):
        ...
        for batch_id, mel_db_batch in enumerate(train_loader):
            ...
            mel_db_batch = torch.reshape(mel_db_batch,
                                         (param.train.batch * param.train.utters, mel_db_batch.size(2),
                                          mel_db_batch.size(3)))
            perm = random.sample(range(0, param.train.batch * param.train.utters),
                                 param.train.batch * param.train.utters)
            unperm = list(perm)
            ...
            embeddings = torch.reshape(embeddings, (param.train.batch, param.train.utters, embeddings.size(1)))
            ...
            if (batch_id + 1) % param.train.log_interval == 0:
                mesg = "{0}\tEpoch:{1}[{2}/{3}],Iteration:{4}" \
                       "\tLoss:{5:.4f}\tTLoss:{6:.4f}\t\n".format(time.ctime(),
                                                                  e + 1,
                                                                  batch_id + 1,
                                                                  len(train_dataset) // param.train.batch,
                                                                  iteration, loss,
                                                                  total_loss / (
                                                                          batch_id + 1))
                print(mesg)
